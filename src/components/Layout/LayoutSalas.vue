...
const validateName = () => {
  if (!name.value) {
    nameErrorMessages.value = ['O nome é obrigatório.']
  } else {
...
const validateQuantidade = () => {
  if (!quantidade.value) {
    quantidadeErrorMessages.value = ['A quantidade máxima de alunos é obrigatória.']
  } else {
...
const saveData = () => {
  ...
  if (nameErrorMessages.value.length === 0 && quantidadeErrorMessages.value.length === 0) {
    // Simulando salvar dados e mostrando a mensagem de sucesso
